        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {services.map((service, index) => (
            <motion.div
              ...
              className="bg-gradient-to-br from-gray-800 to-gray-900 rounded-2xl shadow-xl p-6 text-center hover:from-gray-700 hover:to-gray-800 transition duration-300 transform hover:-translate-y-1 hover:scale-102 relative overflow-hidden"
            >
              ...
              <h3 className="text-xl font-bold mb-3 text-yellow-300">{service.title}</h3>
              <p className="text-gray-300 text-sm leading-relaxed">{service.description}</p>
              <motion.button
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
                className="mt-4 px-4 py-1 bg-yellow-500 text-gray-900 rounded-full font-semibold text-sm hover:bg-yellow-400 transition duration-300"
              >
                Learn More
              </motion.button>
            </motion.div>
